...
在启动子区，抑制性组蛋白修饰，如H3K27me3，通常会使染色质处于紧密状态，很多情况下该区域可能没有明显的抑制性组蛋白修饰富集，所以直接进行 CUT&Tag 实验可能较难获得有生物学意义的 peak。    
​​Tn5 随机切割与污染导致假阳性​​：在 CUT&Tag 实验中，Tn5 转座酶负责切割染色质并插入测序接头。如果实验操作过程中存在污染，例如样本被外源核酸、蛋白质污染，或者实验试剂不纯等，可能会影响 Tn5 的正常切割活性和特异性。Tn5 可能会随机切割染色质，产生一些非特异性的 DNA 片段。这些非特异性片段在后续的文库构建和测序过程中，经过 PCR 扩增等步骤，信号可能会被放大，从而在 TSS（转录起始位点）区域出现类似 peak 的信号，但实际上这并非是真正的抑制性组蛋白修饰富集产生的信号。    
